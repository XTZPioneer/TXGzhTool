...
+ (void)uploadImageWithWithAccessToken:(NSString*)accessToken imageData:(NSData*)imageData article:(TXArticle*)article completionBlock:(CompletionBlock)completionBlock  errorBlock:(ErrorBlock)errorBlock{
    ...
    [TXWXNetWork uploadWithUrl:[NSString stringWithFormat:@"https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=%@",accessToken] parameters:@{@"type" : @"image"} filename:[NSString stringWithFormat:@"%@.png",today_time] mimeType:@"png" fileData:imageData progressBlock:^(NSProgress *progress) {
...
+ (void)uploadArticleWithAccessToken:(NSString*)accessToken article:(TXArticle*)article completionBlock:(CompletionBlock)completionBlock  errorBlock:(ErrorBlock)errorBlock{
    ...
    [TXWXNetWork POST:[NSString stringWithFormat:@"https://api.weixin.qq.com/cgi-bin/material/add_news?access_token=%@",accessToken] parameters:dict progressBlock:^(NSProgress *progress) {
    } returnValueBlock:^(id returnValue) {
        if (returnValue) {
            if (completionBlock) {
